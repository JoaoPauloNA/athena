...
- **Router fallback safety**: `router.run_combo()` now requires positively confirmed termination of the previous attempt (direct process, and Athena's owned process group when a `pgid` existed) before starting a new attempt or falling back to the next provider, raising `FallbackBlocked` or `ComboDeadlineExceeded` instead of risking two concurrent processes against the same workspace.
...
- **Optional Moiras shadow observation** (`athena/moiras_adapter.py`, `mcp_server.py`): explicit `ATHENA_MOIRAS_SHADOW` opt-in submits allowlisted lifecycle metadata to a bounded, coalescing background sampler and exposes the latest current-attempt advisory as `get_execution.execution.moiras_shadow`. The adapter requires Moiras `0.1.x` / schema `1.0`; advisories remain asynchronous, process-local and fixed to `affects_control_flow=false`, `executed=false`, `mode=shadow`. Athena's standard lifecycle source currently reaches four of the six Sentinel classes; wait/external-block classifications remain contract inputs without a standard Athena producer.

### Changed
- `athena/bridge.py`: subprocess/PTY execution now drives `execution.py` state transitions end-to-end, enforces the idle deadline alongside the absolute one, and only reports a terminal state after cleanup is confirmed or explicitly marked unconfirmed. POSIX teardown also performs conservative positive detection of currently visible descendants that escaped the owned group; this blocks confirmation when observed but is not a universal no-escape guarantee.
...
- `athena/reliability.py`: verdict records now categorize free-text termination/cancellation reasons into a fixed, redacted code list before persisting to `~/.athena/verdicts.json`; persistence is best-effort and cannot override a completed execution result.

### Fixed
- **Execution-registry finalization for redacted request IDs**: long-running calls now resolve ownership through the registry's raw request-id index rather than comparing a sanitized/hash public representation with the original JSON-RPC id, preventing finalized records from accumulating until the registry cap.
- **Late-exception workspace lease handling**: the router captures lifecycle snapshots locally and releases after an exception only when the current execution/attempt has authenticated terminal metadata. Indeterminate exceptions and `FallbackBlocked` remain fail-closed with the lease retained. Usage and verdict telemetry failures are best-effort and no longer discard a valid terminal result.
