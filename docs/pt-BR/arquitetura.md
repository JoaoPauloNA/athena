...
| `moiras_adapter.py` | Observador sombra opcional e desativado por padrão. Quando o servidor MCP inicia com `ATHENA_MOIRAS_SHADOW=1`, seu `submit()` seguro para callback coalesce atualizações allowlisted para um sampler limitado em background; `get_execution` pode então expor o advisory inerte da tentativa atual. O Athena nunca o consome para controlar execução. |
...
Em timeout ou cancelamento, o `bridge.py` envia `SIGTERM` ao grupo de processos, espera uma janela de graça (padrão 3s) e escala para `SIGKILL`, só reportando a tentativa como `CANCELLED`/`TIMED_OUT` depois que o processo e o grupo controlado são confirmados positivamente como vazios. Se isso não puder ser confirmado, o estado é `TERMINATION_UNCONFIRMED`, bloqueando fallback automático e liberação do lease. Em POSIX, `start_new_session=True` faz o processo lançado virar líder do próprio grupo. Durante a terminação o Athena também obtém um snapshot best-effort da tabela de processos: se enxergar positivamente um descendente que escapou via `setsid()`/`setpgid()`, recusa a confirmação do escopo controlado. Isso é detecção positiva conservadora, não contenção universal nem prova de ausência; um escape curto ou já reparentado pode não ser observado. Os metadados agora publicam `termination_scope` (`no_process`, `direct_process` ou `owned_process_group`) para que o campo legado `process_tree_terminated_confirmed` não seja confundido com prova sobre todos os descendentes do SO. No Windows não existe garantia equivalente de grupo — ver [Suporte por plataforma](#suporte-por-plataforma).

O `mcp_server.py` registra um `execution_id` por chamada de `run_combo`/`ask_provider` (as duas ferramentas de execução longa) e transmite atualizações de tentativa para o `execution_registry.py`, de onde `get_execution`, `list_executions` e `cancel_execution` leem. `cancel_execution` é idempotente e funciona por `execution_id` ou pelo `request_id` original do JSON-RPC. IDs de request crus continuam como chaves privadas de consulta mesmo quando sua representação pública é hashada.
...
`athena/moiras_adapter.py` é a única fronteira implementada com o projeto independente [Moiras](https://github.com/JoaoPauloNA/moiras). A integração MCP fica desligada por padrão. Instalar o extra opcional `moiras` e definir `ATHENA_MOIRAS_SHADOW` como `1`, `true`, `yes` ou `on` antes de iniciar o servidor cria um sampler limitado (uma thread, intervalo de um segundo; os stores por tentativa têm teto padrão de 64 entradas). Callbacks de lifecycle apenas copiam uma allowlist para um conjunto pendente coalescido; não importam nem chamam Moiras no caminho de dreno de stdout/stderr. Snapshots não terminais inalterados continuam pendentes para tornar o tempo de inatividade observável; snapshots terminais saem depois da amostragem. O encerramento solicita a parada do sampler.

Só cruzam a fronteira IDs de execução/tentativa, estado de lifecycle, marcadores de progresso/estado/histórico, contadores sintéticos, perfil fixo `athena-shadow`, timestamps, revisão de artefato e booleanos explícitos de espera/bloqueio. Provider, prompt, output, comando, path, host, usuário, PID e PGID não cruzam. O contrato aceita Moiras `0.1.x` com `SCHEMA_VERSION == "1.0"`, as funções exigidas e o enum Sentinel exato de seis classes; ausência ou incompatibilidade produz advisory categórico inerte sem texto de exceção. A dependência opcional aponta para uma tag compatível do Moiras.

Com snapshots produzidos pelo `ExecutionRecord` padrão do Athena, quatro classificações são alcançáveis: `REAL_PROGRESS`, `ACTIVITY_WITHOUT_PROGRESS`, `PROBABLE_INACTIVITY` e `INDETERMINATE`. `LEGITIMATE_WAIT` exige `waiting_for_authorization` ou `waiting_for_credential`, e `EXTERNAL_BLOCK` exige `external_block`; esses inputs continuam suportados na fronteira, mas o lifecycle padrão do Athena ainda não emite nenhuma das três flags. Elas não devem ser anunciadas como observadas na execução normal antes de existir um produtor.

Quando ativado, `get_execution` adiciona `execution.moiras_shadow` para a tentativa atual: o advisory mais recente ou `null` enquanto nenhum estiver pronto. A amostragem é assíncrona e intraprocesso, então o valor pode atrasar em relação ao evento mais recente e não é persistido. Todo advisory fixa `affects_control_flow=false`, `executed=false`, `mode=shadow`; não é decisão de conselho/modelo e não relaxa lifecycle, fallback, cancelamento, autorização ou lease.
...
A *detecção* de CLIs roda em macOS, Windows e Linux (ver abaixo). A limpeza do grupo de processos é exercitada pela suíte atual **só em POSIX (macOS/Linux)**. A checagem best-effort da tabela de processos pode detectar positivamente um descendente ainda visível que escapou do grupo e então falhar fechado, mas não prova que nenhum escape ocorreu. O `harness/p0_gate.py` registra esse escopo limitado: `posix` é `LOCAL_CONTROLLED_ONLY`, enquanto `windows` é `NOT_GUARANTEED` (o Athena controla o filho direto, sem alegar limpeza confirmada da árvore mais ampla).
