...
- **EG-3/EG-4 — decisão documental canônica (Vault, 2026-08-26)**: EG-3A deve usar sink interno opt-in e manter intactas as cinco tools MCP; EG-3B fica reservado para eventual exposição pública versionada; EG-4A é somente advisory, não pode emitir nem converter resultado em `PASS`; a saída permanece `awaiting_human_review`. A produção atual já chama o motor, mas viola a decisão internal-only ao acrescentar `payload["evidence_gate"]`; a próxima fatia de código deve reconciliar esse desvio sem alterar outputs ou schemas públicos. Não declarar a correção como concluída antes dessa fatia. Fonte canônica: `Projetos/Carreira/Athena/Evidence-Gate-Integracao-EG3-EG4.md` no Vault.
...
- Gate 4 permanece **NO-GO** para implementação além da próxima correção explicitamente sequenciada: mover EG-3A para sink interno e remover `payload["evidence_gate"]` dos outputs públicos sem mudar tools ou schemas.
- Checkpoint documental G4-0 fechado em 2026-08-27: os dois documentos canônicos do Vault foram sincronizados e validados; a correção de código EG-3A permanece pendente e explicitamente sequenciada.
